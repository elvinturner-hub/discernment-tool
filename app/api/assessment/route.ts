...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updateResult = await db.collection(COLLECTIONS.PROGRESS).updateOne(
      { userId, module, 'answers.questionId': questionId } as any,
      { $set: { 'answers.$.value': value, 'answers.$.answeredAt': new Date(), currentQuestionIndex, updatedAt: new Date() } }
    )
    if (updateResult.matchedCount === 0) {
      await db.collection(COLLECTIONS.PROGRESS).updateOne(
        { userId, module } as any,
        { $push: { answers: answer } as any, $set: { currentQuestionIndex, updatedAt: new Date() }, $setOnInsert: { startedAt: new Date(), completed: false } },
        { upsert: true }
      )
    }
